...
final class HealthKitSyncManager: ObservableObject {
    ...
    func resume() async {
        ...
        do {
            // Background delivery registrations are process-independent, but
            // re-registering on foreground recovery repairs registrations
            // lost after authorization or OS lifecycle changes.
            try await enableBackgroundDelivery()
        } catch {
            state = .failed(error.localizedDescription)
            return
