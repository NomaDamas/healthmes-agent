...
DEMO_NAMESPACE = "healthmes-demo-seed:apple-unified-dashboard:v1"
DEMO_DEDUP_PREFIX = f"{DEMO_NAMESPACE}:trigger:"
DEMO_CAL_PREFIX = f"{DEMO_NAMESPACE}:calendar:"
DEMO_TREE_FLAG = "_healthmes_demo_seed"
...
DEMO_GOAL_IDS = (
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000011"),
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000012"),
)
DEMO_TASK_IDS = (
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000021"),
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000022"),
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000023"),
)
DEMO_INSIGHT_IDS = (
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000031"),
    uuid.UUID("d0d0d0d0-0000-4a1e-8a1e-000000000032"),
)
...
def wipe(session) -> int:
    ...
    demo_decisions = [
        d
        for d in session.query(DecisionRecord).all()
        if isinstance(d.tree, dict) and d.tree.get(DEMO_TREE_FLAG)
    ]
    demo_ids = [d.id for d in demo_decisions]
    if demo_ids:
        n += (
            session.query(ScheduleProposal)
            .filter(ScheduleProposal.decision_record_id.in_(demo_ids))
    ...
    n += (
        ...
        .filter(CalendarEventMirror.external_id.like(f"{DEMO_CAL_PREFIX}%"))
        .delete(synchronize_session=False)
    )
    n += (
        session.query(TriggerEvent)
        .filter(TriggerEvent.dedup_key.like(f"{DEMO_DEDUP_PREFIX}%"))
        .delete(synchronize_session=False)
    )
    n += session.query(Insight).filter(
        Insight.id.in_(DEMO_INSIGHT_IDS)
    ).delete(synchronize_session=False)
    ...
    demo_energy = [
        e
        for e in session.query(CognitiveEnergyEstimate)
        .filter(
            CognitiveEnergyEstimate.window_start >= week_ago_utc0,
            CognitiveEnergyEstimate.inputs_snapshot.isnot(None),
        )
        .all()
        if isinstance(e.inputs_snapshot, dict)
        and e.inputs_snapshot.get("demo_seed") == DEMO_NAMESPACE
    ]
...
def seed(session) -> dict[str, str]:
    ...
    demo_anchor = now_local.replace(second=0, microsecond=0)
    ...
    def block(
        ...
        return CalendarEventMirror(
            ...
            summary=summary,
            start_at=naive_utc(start_local),
            end_at=naive_utc(end_local),
            is_agent_created=agent,
    ...
    session.add_all(
        [
            block(
                "팀 스탠드업",
                demo_anchor - timedelta(minutes=15),
                demo_anchor + timedelta(minutes=15),
            ),
            block(
                "제품 발표 세션",
                demo_anchor + timedelta(minutes=30),
                demo_anchor + timedelta(minutes=90),
            ),
            block(
                "발표 리허설 (에이전트 배치)",
                demo_anchor + timedelta(hours=2),
                demo_anchor + timedelta(hours=3, minutes=30),
                agent=True,
                task_id=t_rehearse.id,
            ),
        ]
    )
    ...
    tree = {
        ...
        "id": "root",
        "type": "rule",
        "label": "stress_spike_vs_baseline 트리거",
        ...
        "children": [
            ...
            },
            {
                "id": "in3",
                "type": "input",
                "label": "오후 캘린더 부하 2.5h + 컨텍스트 스위치 3회",
                "detail": "calendar_event_mirror 조인",
                "children": [],
            },
            {
                "id": "q1",
                                ...
                                "children": [
                                    {
                                        "id": "q3a",
                                        "type": "option",
                                        "label": "내일 오전 09-11시로 이동",
                                        "edge_label": "예",
                                "detail": (
                                    "채택: 에너지 예보 84로 최고 구간, 마감 여유 확보"
                                ),
                                        "children": [],
                                    },
                                    {
            ...
            },
            {
                "id": "act1",
                "type": "action",
                "label": "propose_schedule_blocks 호출",
                "detail": (
    ...
    session.add(
        ScheduleProposal(
            ...
            proposed_start=naive_utc(demo_anchor + timedelta(hours=2)),
            proposed_end=naive_utc(demo_anchor + timedelta(hours=3, minutes=30)),
            status=ProposalStatus.PROPOSED,
            decision_record_id=decision.id,
            reply_handle_digest=hashlib.sha256(
                f"healthmes-demo:{DEMO_ALERT_ID}".encode()
            ).hexdigest(),
            expires_at=naive_utc(demo_anchor + timedelta(hours=4)),
        )
    )
    ...
    feedback_tree = {
        ...
        "id": "root",
        "type": "rule",
        "label": "저녁 리뷰 브리핑 (21:30)",
        "detail": "cron 브리핑 — 오늘 하루 피드백",
        "children": [
            {
                "id": "f1",
                "type": "input",
                "label": "오전 에너지 84-88로 최고 구간",
                "detail": "cognitive_energy_estimate 09-10시",
                "children": [],
            },
            {
                "id": "f2",
                "type": "input",
                "label": "리허설(11:00)을 최고 에너지 직후 배치",
                "detail": "energy_demand=high 태스크 배치 룰 적중",
                "children": [],
            },
            {
                "id": "f3",
                "type": "input",
                "label": "14시 발표 전후 스트레스 스파이크 1회",
                "detail": "trigger_event stress_spike_vs_baseline",
                "children": [],
            },
            {
                "id": "fj",
                ...
                "children": [
                    {
                        ...
                        "detail": "propose_schedule_blocks 예약",
                        "children": [],
                    },
                ],
            },
    ...
    trigger = TriggerEvent(
            ...
            payload={
                ...
                "proposal": (
                    "16:00 집중 블록을 내일 오전으로 옮기고 "
                    "오후는 가벼운 정리만 배치할게요"
                ),
                "evidence": {"stress": 71, "baseline": 51.5, "sleep_score_last_night": 68},
            },
            alert_sent=True,
            dedup_key=f"{DEMO_DEDUP_PREFIX}{today.isoformat()}",
    )
    session.add(trigger)
    session.flush()
    decision.trigger_event_id = trigger.id
        ...
        for local_hour, base_score in ENERGY_BY_LOCAL_HOUR.items():
            ...
            session.add(
                CognitiveEnergyEstimate(
                    ...
                    inputs_snapshot={
                        "demo_seed": DEMO_NAMESPACE,
                        "source": "scripts/demo_seed.py",
                    },
                )
            )
    ...
    session.add_all(
        [
            Insight(
                id=DEMO_INSIGHT_IDS[0],
                period=period,
                kind="focus_dip",
                statement=INSIGHT_STATEMENTS[0],
                evidence={"window": "14-16h", "n_days": 12, "ratio": 1.8},
                confidence=0.68,
            ),
            Insight(
                id=DEMO_INSIGHT_IDS[1],
                period=period,
                kind="factor_correlation",
                statement=INSIGHT_STATEMENTS[1],
                evidence={"factor": "morning_run", "delta_pct": -12, "n": 8},
                confidence=0.62,
            ),
        ]
    )
