...
```
[워치/폰 11개 프로바이더] → open-wearables (데이터 플레인, 확장점 우선)
                                  │ REST (원시·점수 데이터)
                                  ▼
...
**벤더 경계**: 메트릭과 정책은 기본적으로 HealthMes에 둡니다. 다만
Open Wearables 또는 Hermes에 필요한 원천 기능이 공개 계약과 HealthMes
글루만으로 안전하게 제공되지 않으면, 최소 vendor 패치와 회귀 테스트를
`vendor(ow):` 또는 `vendor(hermes):` 독립 커밋으로 반입할 수 있습니다.
범용 수정은 upstream PR로도 제안합니다.
