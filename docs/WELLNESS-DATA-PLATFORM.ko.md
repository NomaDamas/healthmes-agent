...
> **상태:** 2026-08-05 소유자 요청에 따른 해자 전략 보강 및 딥리서치 결과.
>
...
HealthMes의 여러 해자 중 이 문서가 정의하는 해자는 다음 네 가지다.
...
권한·품질·보존 정책 아래 계속 수용하는 플랫폼 전체는 복제 비용이 커진다.

저장 정책은 모든 데이터를 7일 또는 14일 뒤 지우는 단일 TTL보다 **원본은 짧게,
요약과 결과 그래프는 길게** 보존하는 계층형 정책이 적합하다.

저장 위치, 모바일·노트북 역할, iCloud, 데이터별 TTL, 클라우드 과금 및
worktree 격리의 기준은
...
┌─────────────────────────────────────────────────────────────┐
│ 입력 어댑터                                                  │
│ Open Wearables · HealthKit · Health Connect · 음식 · 앱사용 │
...
| 음식·증상 원본 사진/음성 | 7일 | 민감도와 용량이 큼; 확인 뒤 구조화값 유지 |
...
- RemoteVault에 조직 정책, 보존 잠금, 무결성 검증, 사용량·과금 계층을 얹는다.
- 고객별 키 또는 고객 관리 키(BYOK)를 지원하되 평문 분석을 기본값으로 만들지 않는다.

**종료 조건:** 저장 용량 판매가 HealthMes의 평문 데이터 접근 권한 판매로
