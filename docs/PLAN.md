...
| 애플워치 연속 수집 | 폰이 알아서 주기 업로드 | 브리지 수신 어댑터: 기성 HealthKit 자동 내보내기 앱(예: Health Auto Export)의 REST POST를 받아 OW 인입 계약으로 변환하는 glue 엔드포인트. 앱 코드 0줄 | ⏭ 다음 작업 |
...
| 알림 | 설정 0 | `native_alert_delivery` + 빌드된 컴패니언 폴링, Telegram은 옵션으로 강등 | ✅ (기본값 전환만) |
