...
### 판단 원칙 (스트레스 스킬 §개정에 반영)

1. **연속 반영** — 유효한 신호는 강도만큼 **항상** 점수·배치·설명에 반영된다.
   "몇 표 이상이어야 고려" 같은 문턱은 없다 (인지에너지 엔진의 가중+재정규화
   철학을 판단 스킬까지 확장).
2. **근거 그룹** — 같은 측정에서 파생된 신호(야간 HRV·회복탄력성·charge 등)는
   한 그룹 = 최대 1개의 독립 근거. 측정 1번을 여러 표로 세는 과신 금지.
3. **개입 사다리** — 증거의 양이 아니라 **개입의 세기**가 단계적:
   L1 언급만 → L2 가역적 제안 1개(강한 단일 신호로도 도달) → L3 강한 권고
   (서로 다른 그룹 2개 이상). 알림 남발 방지는 기존 알림 위생(예산·쿨다운) 담당.
