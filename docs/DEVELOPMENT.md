...
| `POST /v1/schedule/proposals/{id}/accept` / `/decline` | bearer | The apps' ✅/❌ actions. JSON body requires the proposal's one-time `{"reply_handle":"..."}`. Missing body → 422, invalid handle → `403 invalid_reply_handle`, expired handle → `409 proposal_expired`, second tap → `409 invalid_transition` with `detail {current, requested}` (render "already resolved"); unknown id → 404. |
