...
| `POST /v1/nutrition-observations/analyze` | bearer | Accepts an uploaded image `media_path` plus capture time/timezone/provenance. Calls the configured Ollama vision model with a strict JSON schema and stores sake's `NutritionObservation` intact in `WellnessEvent.payload`. Default provider is loopback-only; remote use requires per-request `allow_remote_vision: true` and HTTPS. Repeating the same media returns the existing observation. |
