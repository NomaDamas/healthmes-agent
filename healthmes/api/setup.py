...
from healthmes.wearables.management import build_management_client
...
def _valid_open_wearables_user_id(value: object) -> bool:
...
def _injected_management_client(request: Request) -> Any | None:
    """Return a test/runtime management client already attached to the app."""

    client = getattr(request.app.state, "wearable_management_client", None)
    if client is not None:
        return client
    return getattr(request.app.state, "ow_management_client", None)


async def resolve_management_client(
    request: Request,
) -> tuple[Any | None, Exception | None]:
    """Resolve one Open Wearables client for a complete settings request.

    ``build_management_client`` applies the same single-user policy used by
    the Main MCP/decision paths. Returning the error separately lets callers
    reuse the failed resolution without making a second upstream request.
    """

    settings: Settings = request.app.state.settings
    if not settings.ow_api_key.get_secret_value().strip():
        return None, None
    injected = _injected_management_client(request)
    if injected is not None:
        if not _valid_open_wearables_user_id(
            getattr(injected, "user_id", None)
        ):
            return None, ValueError(
                "Open Wearables user could not be resolved safely."
            )
        return injected, None
    try:
        return await build_management_client(settings), None
    except Exception as exc:
        return None, exc


async def build_setup_readiness(
    request: Request,
    descriptors: Iterable[InputSourceDescriptor],
    *,
    management_client: Any | None = None,
    management_error: Exception | None = None,
) -> SetupReadinessOut:
    """Compose existing server controls into the Apple setup projection.

    The Open Wearables check is not satisfied by an API key alone. The
    management client must have resolved the same single user that the Main
    decision and MCP paths read.
    """

    settings: Settings = request.app.state.settings
    if (
        management_client is None
        and management_error is None
        and settings.ow_api_key.get_secret_value().strip()
    ):
        management_client, management_error = await resolve_management_client(
            request
        )
    descriptors_by_id = {
        item.source_id: item
    ...
    open_wearables_ready = bool(
        ...
        and management_error is None
        and management_client is not None
        and _valid_open_wearables_user_id(
            getattr(management_client, "user_id", None)
        )
    )
    ...
    checks = [
        ...
        _source_check(
            ...
            action_detail=(
                "Configure the Open Wearables API key and resolve exactly one "
                "user, or set HEALTHMES_OW_USER_ID."
            ),
...
@router.get("/readiness", response_model=SetupReadinessOut)
async def setup_readiness(
    ...
    readiness = await build_setup_readiness(
        request,
        InputSourceRegistry(settings=settings).list(session),
