...
from healthmes.api import setup
...
def test_readiness_composes_existing_input_and_runtime_controls(
    client,
    monkeypatch,
) -> None:
    async def unresolved(_settings):
        raise WearableManagementConfigurationError(
            "Open Wearables user could not be resolved safely."
        )

    monkeypatch.setattr(setup, "build_management_client", unresolved)
...
def test_readiness_does_not_mark_configured_placeholders_ready(
    client,
    monkeypatch,
) -> None:
    async def unresolved(_settings):
...
def test_readiness_requires_a_resolved_single_wearables_user(
    client,
) -> None:
    client.app.state.wearable_management_client = SimpleNamespace(
        user_id="0b6f3a52-8c1d-4e2a-9f10-2a5b7c9d1e3f"
    )
    ...
    assert response.status_code == 200
    checks = {check["key"]: check for check in response.json()["checks"]}
    assert checks["open_wearables"]["state"] == "ready"
    assert "0b6f3a52" not in response.text
    assert "test-ow-api-key" not in response.text


def test_readiness_rejects_an_invalid_injected_wearables_user(
    client,
) -> None:
    client.app.state.wearable_management_client = SimpleNamespace(
    ...
    checks = {check["key"]: check for check in response.json()["checks"]}
    assert checks["open_wearables"]["state"] == "action_required"
    assert "not-a-canonical-user-id" not in response.text
    assert "test-ow-api-key" not in response.text
