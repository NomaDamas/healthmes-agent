...
Shell scripts get no import-time checking, so the invariants that prevent
setup/runtime from mutating vendor source or lockfiles are pinned here as
text/syntax assertions.
"""
...
def test_ow_env_freezes_uv_against_vendor_lock_rewrites() -> None:
    ...
    vendor/open-wearables/backend/uv.lock on pyproject drift. (The hermes
    config template and the ow-mcp compose service pass --frozen explicitly;
    this is the same guarantee for the mac-native path.)"""
